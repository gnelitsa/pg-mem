  ...
  return knex.schema
    ...
    .createTable("room_types_characteristics", function (table) {
      table
        ...
        .inTable("room_types");
      table
        .string("characteristic_id", 10)
        .notNullable()
        .references("id")
        .inTable("room_characteristics");
...
describe("IRL tests", () => {
  it("carlosfaria94 x Knex migration fails", async () => {
    ...
    mem.public.none(
      `insert into room_characteristics(id,name,is_enabled) values ('roomid', 'roomname', true)`
    );

    const result = await knex("room_characteristics").select(
      "id",
      "is_enabled"
    );
